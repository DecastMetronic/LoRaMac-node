...
/*!
 * LoRaMac ADR ack counter limit
 */
static uint8_t AdrAckLimit = RU864_ADR_ACK_LIMIT;

/*!
 * LoRaMac ADR ack counter delay
 */
static uint8_t AdrAckDelay = RU864_ADR_ACK_DELAY;
...
bool RegionRU864AdrNext( AdrNextParams_t* adrNext, int8_t* drOut, int8_t* txPowOut, uint32_t* adrAckCounter )
{
    ...
    *adrAckCounter = adrNext->AdrAckCounter;
    ...
    if( adrNext->AdrEnabled == true )
    {
        ...
        {
            if( adrNext->AdrAckCounter >= AdrAckLimit )
            {
                adrAckReq = true;
            ...
            if( adrNext->AdrAckCounter >= ( AdrAckLimit + AdrAckDelay ) )
            {
                if( ( adrNext->AdrAckCounter % AdrAckDelay ) == 1 )
                {
...
bool RegionRU864RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
    ...
    {
        modem = MODEM_LORA;
        Radio.SetRxConfig( modem, rxConfig->Bandwidth, phyDr, RU864_CODING_RATE, 0, 8, rxConfig->WindowTimeout, false, 0, false, 0, 0, true, rxConfig->RxContinuous );
    }
...
bool RegionRU864TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    {
        modem = MODEM_LORA;
        Radio.SetTxConfig( modem, phyTxPower, 0, bandwidth, phyDr, RU864_CODING_RATE, 8, false, true, 0, 0, false, 3000 );
    }
